P_Boeing777_2022_09_14: ; -> PMDG 777-200LR / 777-F
...
DEV_VARS:
  ...
  Global Aktu_Scenario := "Boeing777" ; zum suchen in Load Scenario
  ...
  Global ANZ_Flaps := 6 ; 1; 5; 15; 20; 25; 30
  Global STR_Flaps := ["FUP", "F01", "F05", "F15", "F20", "F25", "F30"]
...
CMD_Process:
  ...
  Else If (CMD_Text="flight director on")
  {
    If Not FD
    {
      ...
      Send {Shift Down}{Ctrl Down}{Alt Down}f{Alt Up}{Shift Up}{Ctrl Up} ; FD rechts und links über P3DKey Ctrl f
    }
    Err := _Text_to_Speech(CMD_Text)
  }
  ...
  Else If (CMD_Text="autothrust on")
  {
    If Not AT
    {
      ; Send {Ctrl Down}r{Ctrl Up} ; arm/ disarm AT
  ...
  Else If (CMD_Text="managed speed")
  {
  ...
  Else If (CMD_Text="managed heading")
  {
  ...
  Else If (CMD_Text="change flightlevel")
  {
    Send {Shift Down}{Ctrl Down}{Alt Down}c{Alt Up}{Shift Up}{Ctrl Up}
    Err := _Text_to_Speech(CMD_Text)
  }
  ;
  ; Controls
  ...
  {
    Send {Shift Down}{Ctrl Down}{Alt Down}{Pgup 6}{Alt Up}{Shift Up}{Ctrl Up}
    Err := _Text_to_Speech(CMD_Text)
  }
  Else If (CMD_Text="autobrake off")
  {
    Send {Shift Down}{Ctrl Down}{Alt Down}{Pgdn 6}{Alt Up}{Shift Up}{Ctrl Up}
    Err := _Text_to_Speech(CMD_Text)
  }
  Else If (CMD_Text="autobrake RTO")
  {
    Send {Shift Down}{Ctrl Down}{Alt Down}{Pgdn 1}{Alt Up}{Shift Up}{Ctrl Up}
    Err := _Text_to_Speech(CMD_Text)
  }
  ...
  Else If (CMD_Text="go around")
  {
    ...
    If GEAR
      Send g
    Send {F5}{F7}{F7}{F7} ; Flaps 20
    Err := _Text_to_Speech(CMD_Text)
  }
  Else If (CMD_Text="warnings off")
  {
    Send ^w^w^w^w^w ; Master Warnings
    Err := _Text_to_Speech(CMD_Text)
  }
  Else If (CMD_Text="switch radio") ; FSUIPC KEY DEFINITION (Com Stby Radio Swap )
  {
  ...
  Else If (CMD_Text="strobe and landing lights on") ; LUA Functions
  {
    Send {Shift Down}{Ctrl Down}{Alt Down}s{Alt Up}{Shift Up}{Ctrl Up}
    Send {Shift Down}{Ctrl Down}{Alt Down}l{Alt Up}{Shift Up}{Ctrl Up}
    Err := _Text_to_Speech(CMD_Text)
  }
  Else If (CMD_Text="strobe and landing lights off") ; LUA Functions
  {
    Send {Ctrl Down}{Alt Down}s{Alt Up}{Shift Up}{Ctrl Up}
    Send {Ctrl Down}{Alt Down}l{Alt Up}{Shift Up}{Ctrl Up}
    Err := _Text_to_Speech(CMD_Text)
  ...
  Else If (CMD_Text="Aircraft Scenario on")
  {
...
_Preflight_Procedure() { ; Starts manually
  ...
  If _Is_CheckItem("Battery on, guard closed!")
    ...
    ; If _Is_CheckItem("Battery on, guard closed!") ; electrical power up
  ; If _Is_CheckItem("If ground power available, ground power switch on!")
  ; If _Is_CheckItem("Start APU generator!")
  ; If _Is_CheckItem("Standby power auto, guard closed!")
  ; If _Is_CheckItem("Bus transfer auto, guard closed!")
  ...
  If _Is_CheckItem("Set engine hydraulic pumps to on position!")
    If _Is_CheckItem("Set fuel pumps on!")
    If _Is_CheckItem("Set left and right packs in auto position!")
    If _Is_CheckItem("Set APU bleed switch to auto!")
    If _Is_CheckItem("Switch IRS on!")
    If _Is_CheckItem("Set Seat belt sign and no electronics to auto!")
    ; If _Is_CheckItem("Yaw dampers on!")
  If _Is_CheckItem("Prepare the CDU!")
    If _Is_CheckItem("Set logo, position and wing lights on!")
...
_Preflight_Checklist() { ; Starts manually
  ...
  ; If _Is_CheckItem("Electrical Hydraulic Pums swiched On!")
  ...
  ; If _Is_CheckItem("PressurizatiOn Panel AUTO?")
  ; If _Is_CheckItem("Anti CollisiOns Light On?")
  ; If _Is_CheckItem("Preflight checklist is complete! You can REQUEST PUSH BACK and START THE ENGINES!")
...
_BeforeTakeOff_Checklist() { ; Starts when landing lights goes on
  SetTimer,, Off
  CheckList_Active := True

  Err := _Text_to_Speech("Before take off checklist")

  If _Is_CheckItem("Check if autothrust armed!")
    If _Is_CheckItem("Check the laterale navigation!")
...
_AfterTakeOff_Checklist() { ; Starts after flaps full up
  ...
  If _Is_CheckItem("Check if the gear and the flaps are up!")
...
_BeforeApproach_Checklist() { ; Starts after descending 8000 Feets
  ...
  If _Is_CheckItem("Check the CDU approach page and the ILS Frequency!")
    If _Is_CheckItem("Check if the landing lights on!")
    If _Is_CheckItem("Check is the radar altitude set!")
    If _Is_CheckItem("Check if the autobrake is on!")
...
_Read_FS_VARS() {
  ...
  Global PBRAKE := 0x65B4 ; parking brake B777
  ...
  If AC_TYPE_read
  {
    ...
    Else
    {
      _Error_Message("AC_TYPE: B777 ->" AC_TYPE "<", 6)
      AC_TYPE := "????"
    }
  ...
  PBRAKE 	:= NumGet(&PBRAKE, 0, "short") > 0 ; B777
  ; err :=_Message("PBRAKE? " PBRAKE, 0)
...
Aircraft_Scenario:
  ...
  If ((FLAPS_V > 5) And Not BeforeLanding_Ok And BeforeApproach_Ok And Not CheckList_Active)
  {
    If (Not TAXI_L)
...
POV: ; 3Joy
  ...
  If ((Aktu_Screen = 9) Or GetKeyState("LCtrl"))
  {
    ...
    {
      Send {Ctrl Down}{7 %POV_Step%}{Ctrl Up}
    }
    else if (POV_Value = 0) ; Numpad8
    {
      Send {Ctrl Down}{8 %POV_Step%}{Ctrl Up}
    }
    else if (POV_Value = 4500) ; Numpad9
    {
      Send {Ctrl Down}{9 %POV_Step%}{Ctrl Up}
    }
    ...
    {
      Send {Ctrl Down}{6 %POV_Step%}{Ctrl Up}
    }
    else if (POV_Value = 22500) ; Numpad1
    {
      Send {Ctrl Down}{1 %POV_Step%}{Ctrl Up}
    }
    else if (POV_Value = 18000) ;Numpad2
    {
      Send {Ctrl Down}{2 %POV_Step%}{Ctrl Up}
    }
    else if (POV_Value = 13500) ; Numpad3
    {
      Send {Ctrl Down}{3 %POV_Step%}{Ctrl Up}
    }
