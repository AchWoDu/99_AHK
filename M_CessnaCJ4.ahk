...
Aircraft_Scenario:
  {
    ...
    Show_DEBUG_Info:
      ...
      If DEBUG
      {
        ...
        | Scr7: %Screen7_On%
        ; | LIGHTS: %LIGHTS%
        , x_ToolTip10, y_ToolTip10, 10

        ACS_EndTime := A_TickCount - ACS_StartTime
        ACS_Time := ACS_EndTime - RFV_Time - WSB_Time
        ACS_LastTime := ACS_StartTime - ACS_LastTime

        ToolTip,	ACS_LastTime: %ACS_LastTime%
        | RFV_Time: %RFV_Time%
        | WSB_Time: %WSB_Time%
        | ACS_Time: %ACS_Time%
        | ACS_EndTime: %ACS_EndTime%
        , x_ToolTip8, y_ToolTip8, 8

        ACS_LastTime := A_TickCount
      }
      Else
      {
        ToolTip,,,, 8
