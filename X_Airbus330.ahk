...
CMD_Process:
  {
    ...
    Else If (CMD_Text="navigation window")
    {
      ; Send {Shift Down}{Ctrl Down}{Alt Down}o{Alt Up}{Shift Up}{Ctrl Up}
      Send 1
    }
    Else If (CMD_Text="CDU")
    {
      ; Send {Shift Down}{Ctrl Down}{Alt Down}i{Alt Up}{Shift Up}{Ctrl Up}
      Send 2
    }
    ...
    Else If (CMD_Text="great displays") ; 3
    {
      Gosub Screen3
    }
    ...
    Else If (CMD_Text="autothrust on") ; Lua Script
    {
      Send {Shift Down}{Ctrl Down}{Alt Down}j{Shift Up}{Ctrl Up}{Alt Up}
      Err := _Text_to_Speech(CMD_Text)
    }
    Else If (CMD_Text="autothrust off") ; Lua Script
    {
    ...
    {
      Send {F6 4}{F7 1}
      Send {F6 4}{F7 1} ; workaround JAR Problem
      ; Err := _Text_to_Speech(CMD_Text)
    }
    Else If (CMD_Text="flaps 2")
    {
      Send {F6 4}{F7 2}
      Send {F6 4}{F7 2} ; workaround JAR Problem
      ; Err := _Text_to_Speech(CMD_Text)
    }
  ...
  _Preflight_Procedure() ; V1 /"Reload data refs in X U I P C settings!"
  ; Starts manually
  {
    SetTimer,, Off
  ...
  _Read_FS_VARS()
  {
    ...
    If AC_TYPE_read
    {
      AC_TYPE := StrGet(&AC_TYPE, 5, "UTF-8")

      If (AC_TYPE = "JD330")
        AC_TYPE := "A330" ; 4 stellig
      Else
      {
        _Error_Message("AC_TYPE:" AC_TYPE, 6)
        AC_TYPE := "????"
      }
    ...
    Global FD := 0x5510
    Err := DllCall(FSUIPC_LibPfad . "\FSUIPC_Read", uint, FD, uint, 1, short, &FD, uint, &dwResult)

    Global AT := 0x5511
    Err := DllCall(FSUIPC_LibPfad . "\FSUIPC_Read", uint, AT, uint, 1, short, &AT, uint, &dwResult)

    Global AP := 0x5512
    Err := DllCall(FSUIPC_LibPfad . "\FSUIPC_Read", uint, AP, uint, 1, short, &AP, uint, &dwResult)

    ; Global TP := 0x65CD
    ; Err := DllCall(FSUIPC_LibPfad . "\FSUIPC_Read", uint, 0x65CD, uint, 1, uint, &TP, uint, &dwResult)

    Global BEACON_L := 0x5513
    Err := DllCall(FSUIPC_LibPfad . "\FSUIPC_Read", uint, BEACON_L, uint, 1, short, &BEACON_L, uint, &dwResult)

    Global TAXI_L := 0x5514
    Err := DllCall(FSUIPC_LibPfad . "\FSUIPC_Read", uint, TAXI_L, uint, 1, short, &TAXI_L, uint, &dwResult)

    Global STROBE_L := 0x5515
    Err := DllCall(FSUIPC_LibPfad . "\FSUIPC_Read", uint, STROBE_L, uint, 1, short, &STROBE_L, uint, &dwResult)

    Global LAND_L := 0x5516
    Err := DllCall(FSUIPC_LibPfad . "\FSUIPC_Read", uint, LAND_L, uint, 1, short, &LAND_L, uint, &dwResult)

    Global STD := 0x5517
    Err := DllCall(FSUIPC_LibPfad . "\FSUIPC_Read", uint, STD, uint, 1, short, &STD, uint, &dwResult)
